<h1 align="center">Hi, I'm Kushal Sai 👋</h1>

I’m a **3rd Year Computer Science student** interested in building **backend applications** and exploring how **AI-powered systems** work in practice.

- Interested in **backend development and AI**
- Enjoy working with **APIs, databases, and backend logic**
- Like writing **clean, structured, and readable code**
- Currently learning and building projects using **Go for backend development**
- Exploring **AI concepts** by building small, practical projects

## Skills

- **Languages:** Java, Go, Python, JavaScript, TypeScript  
- **Frameworks/Libraries:** React.js, Tailwind CSS, Gin, Node.js, Express.js  
- **Databases & Tools:** PostgreSQL, MongoDB, Docker, Git, Postman, GitHub Actions (CI/CD)

## Connect with Me

📫 **Email:** gkushalsai2006@gmail.com  
📍 **Location:** Bangalore, India

![](https://github-readme-stats.vercel.app/api/top-langs/?username=kushalsai-01&theme=dark&hide_border=true&layout=compact)


